...
function make_array(arrlength){
    ...
    for(var i=0;i<arrlength;i++){
        ...
        blocks[i].style="width:"+ width +"px"+";margin:"+ "0 0.5px" +";background-color: blue"+";height: "+block_sizes[i]+"%" +";top:"+(420-(4.2*block_sizes[i]))+"px";
...
function vis_speed()
{
    ...
    delay_time=100000/(Math.floor(arrlength/10)*speed);        //Decrease numerator to increase speed.
}
var d_delay = 0;
...
buttons.forEach(function(button){
    button.addEventListener("click",function(e){
        if(e.target.textContent=="Bubble") bubble_sort();
        else if (e.target.textContent=="Insertion") insertion_sort();
        else if(e.target.textContent=="Selection") selectionSort();
        else if(e.target.textContent=="Merge") Merge();
        else if(e.target.textContent=="Quick") Quick();
    })
});
